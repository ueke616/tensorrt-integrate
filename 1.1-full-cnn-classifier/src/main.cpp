...
bool build_model(){
    ...
    // 通过 onnxparser 解析器解析的结果会填充到 network 中, 类似addConv的方式添加进去
...
void inference() {
    ...
    std::cout << "类型: " << typeid(engine_data.pop_back()).name() << std::endl;
    ...
    auto exectution_context = make_nvshared(engine->createExecutionContext());
    ...
    // 明白当前推理时, 使用的数据输入大小
    auto input_dims = exectution_context->getBindingDimensions(0); 
    input_dims.d[0] = input_batch;

    // 设置当前推理时，input大小
    exectution_context->setBindingDimensions(0, input_dims);
    float* bindings[]   = {input_data_device, output_data_device};
    bool success        = exectution_context->enqueueV2((void**)bindings, stream, nullptr);
    ...
    float confidence    = prob[predict_label];      // 获取预测值的置信度
