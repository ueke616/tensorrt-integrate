...
bool build_model(){
    ...
    if(exists("yolox-tiny.trtmodel")){
        printf("yolox-tiny.trtmodel has exists.\n");
        return true;
    }
    ...
    if(!parser->parseFromFile("yolox-tiny.onnx", 1)){
        printf("Failed to parse yolox-tiny.onnx\n");
    ...
    FILE* f = fopen("yolox-tiny.trtmodel", "wb");
...
void inference(){
    ...
    TRTLogger logger;
    auto engine_data = load_file("yolox-tiny.trtmodel");
    ...
    printf("the of num of pred bounding box:\n");
    printf("==========before nms============\n");
